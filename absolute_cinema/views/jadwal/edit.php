...
$id = $_GET['id'];
$dataJadwal = $jadwalVM->fetchById($id); // Data yang mau diedit
$films = $filmVM->fetchAll(); // Untuk dropdown
$studios = $studioVM->fetchAll(); // Untuk dropdown

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if ($jadwalVM->updateJadwal($_POST)) {
        header("Location: index.php");
        exit;
    }
}
...
<head>
    ...
    <title>Edit Jadwal - Cinema Tix</title>
            ...
            <div class="col-md-8">
                <div class="card card-custom">
                    <div class="card-header py-3">
                        <i class="fas fa-edit me-2"></i> Edit Jadwal Tayang
                    </div>
                    <div class="card-body p-4">
                        ...
                        <form method="POST">
                                ...
                                <div class="input-group">
                                    ...
                                    <select name="id_film" class="form-select" required>
                                        <?php foreach ($films as $f): ?>
                                            <option value="<?= $f['id'] ?>" <?= ($f['id'] == $dataJadwal->id_film) ? 'selected' : '' ?>>
                                                <?= $f['judul'] ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                            ...
                            <div class="mb-4">
                                ...
                                <div class="input-group">
                                    <span class="input-group-text bg-secondary border-secondary text-white"><i
                                            class="fas fa-couch"></i></span>
                                    <select name="id_studio" class="form-select" required>
                                        <?php foreach ($studios as $s): ?>
                                            <option value="<?= $s['id'] ?>" <?= ($s['id'] == $dataJadwal->id_studio) ? 'selected' : '' ?>>
                                                <?= $s['nama_studio'] ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-4">
                                    ...
                                    <input type="date" name="tgl_tayang" class="form-control"
                                        value="<?= $dataJadwal->tgl_tayang ?>" required>
                                </div>

                                <div class="col-md-6 mb-4">
                                    <label class="form-label">Jam Tayang</label>
                                    <input type="time" name="jam_tayang" class="form-control"
                                        value="<?= $dataJadwal->jam_tayang ?>" required>
                                </div>
                            </div>

                            <div class="d-flex justify-content-end gap-2 mt-3">
                                <a href="index.php" class="btn btn-cancel px-4">Batal</a>
                                <button type="submit" class="btn btn-update px-4">Update Jadwal</button>
                            </div>
