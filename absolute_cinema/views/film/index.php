...
<head>
    ...
    <title>Daftar Film - Cinema Tix</title>
...
<body>
    ...
    <div class="container main-container">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="fw-bold"><i class="fas fa-video text-danger"></i> Daftar Film</h2>
            ...
            <div class="table-responsive">
                <table class="table table-dark-custom table-hover">
                    ...
                    <tbody>
                        <?php if (empty($films)): ?>
                            <tr>
                                <td colspan="6" class="text-center p-5 text-muted">Belum ada data film.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($films as $index => $row): ?>
                                <tr>
                                    <td class="text-center"><?= $index + 1 ?></td>
                                    <td class="fw-bold text-warning"><?= $row['judul'] ?></td>
                                    <td><span class="badge bg-secondary"><?= $row['genre'] ?></span></td>
                                    <td><?= $row['durasi'] ?> Menit</td>
                                    <td class="fw-bold text-success">Rp <?= number_format($row['harga_dasar'], 0, ',', '.') ?>
                                    </td>
                                    <td class="text-center">
                                        ...
                                        <a href="index.php?action=delete&id=<?= $row['id'] ?>" class="btn btn-danger btn-sm"
                                            onclick="return confirm('Yakin ingin menghapus film <?= $row['judul'] ?>?')"
                                            title="Hapus">
