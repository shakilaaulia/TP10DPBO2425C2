...
<head>
    ...
    <title>Data Studio - Cinema Tix</title>
...
<body>
    ...
    <div class="container main-container">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="fw-bold"><i class="fas fa-couch text-danger"></i> Daftar Studio</h2>
                ...
                <table class="table table-dark-custom table-hover">
                    ...
                    <tbody>
                        <?php if (empty($studios)): ?>
                            <tr>
                                <td colspan="4" class="text-center p-5 text-muted">Belum ada data studio.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($studios as $index => $row): ?>
                                <tr>
                                    <td class="text-center"><?= $index + 1 ?></td>
                                    <td class="fw-bold text-warning"><?= $row['nama_studio'] ?></td>
                                    <td><i class="fas fa-chair text-secondary"></i> <?= $row['kapasitas'] ?> Kursi</td>
                                    <td class="text-center">
                                        ...
                                        <a href="index.php?action=delete&id=<?= $row['id'] ?>" class="btn btn-danger btn-sm"
                                            onclick="return confirm('Yakin ingin menghapus studio ini?')" title="Hapus">
